...
const API_URL = "http://localhost:3000/api/login"; // corrigido para /api/login
...
if (!formLogin) {
  ...
  formLogin.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        alert((data && data.message) || "Email ou senha incorretos.");
        setLoading(false);
        return;
      }
      ...
      if (data && data.user) {
        try { sessionStorage.setItem("users", JSON.stringify(data.user)); } catch (err) { console.warn(err); }
      }
